...
**검색 동작(확정 2026-07-07, 사용자 결정)**: 헤더 **셸은 전 페이지 완전 동일**(h-12·로고 96px·인풋 h-8)하되, **/list에서는 검색이 드롭다운(→상세 이동) 대신 리스트 필터(`?name=` 디바운스 반영)로 동작** — 데스크톱의 현행 MainSearch/DetailSearch 분기 패턴을 모바일에도 적용해 데/모 동작을 일치시킨다. "이름으로 목록 좁혀 보기" 기능 유지가 근거. 페이지마다 검색 동작이 달라지는 이원성은 감수(데스크톱 기존 관례).
