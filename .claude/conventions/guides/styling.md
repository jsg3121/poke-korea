...
`tailwind.config.js`의 `screens`에 위 2종 외에 다른 브레이크포인트를 추가하지 않는다. 중간 구간이 필요해 보이면 **레이아웃을 먼저 의심한다** — 대개 열 수나 고정 폭을 조정하면 한 구간으로 전 폭을 커버할 수 있다.

> **Why:** 이름에 `desktop`이 들어가도 `max-width`로 정의하면 모바일까지 포함해, 이름만 보고 데스크톱 전용으로 오해하게 된다. 게다가 Tailwind는 `desktop:narrow-970:` 같은 **중첩 변형에 대해 규칙을 생성하지 않아**, "데스크톱이면서 좁은 구간"을 표현할 수 없다. 두 브레이크포인트가 서로를 좁히지 못하므로 구간을 쪼갤수록 쓸 수 없는 조합만 늘어난다.
>
> 실제 해법은 레이아웃 쪽에 있다. 9열이 좁은 폭에서 깨진다면 6열로 줄여 전 구간에서 안전하게 만드는 편이, 구간을 나눠 각각 다른 열 수를 주는 것보다 단순하고 검증하기 쉽다.
...
| **DS 컴포넌트 우선 사용** | 이미 DS 컴포넌트가 있는 패턴(가로 스크롤=`HorizontalScrollList`, 카드 셸=`PokemonCardShell` 등)은 **반드시 그 컴포넌트를 import해서 쓴다.** DS의 마크업·클래스를 복사해 자체 구현하지 말 것 — DS 규격이 바뀌어도 복사본엔 반영되지 않아 화면이 어긋난다 |
...
| **content 스캔 범위** | `tailwind.config.js`의 `content`에 **클래스를 쓰는 모든 디렉토리**가 포함돼야 한다. 누락된 디렉토리의 파일에 쓴 유틸은 CSS로 생성되지 않아 **조용히 무효화**된다 — HTML엔 클래스가 있으나 스타일이 적용되지 않은 채 렌더된다. 클래스가 안 먹으면 `content`에 그 디렉토리가 있는지 가장 먼저 확인한다. 폴더를 새로 만들거나 이름을 바꿀 때 함께 갱신할 것                                                                                                                                                                                                                              |
