...
본 ADR의 **근거 보고서(`MI-BA-2026-05-26-podic-traffic-overtake.md`)에 사실 오류**가 확인되어 ADR 전체를 폐기한다.

### 오류 내용

BA 분석이 `metrics-baseline.md`의 페이지별 트래픽 TOP 10만 보고 다음 두 페이지를 **"미구현, 신규 생성 필요"** 로 단정했으나, 실제 상용 코드를 확인한 결과 두 페이지 모두 이미 구현되어 있었다.

| 페이지          | BA 보고서 주장                       | 실제 상태                                                                                   |
| --------------- | ------------------------------------ | ------------------------------------------------------------------------------------------- |
| `/moves/[id]`   | "구현 안 됨, 신규 생성 필요 (1순위)" | **구현됨** — `src/app/moves/[id]/page.tsx` (메타데이터, JSON-LD, 반응형, ISR 1년 모두 완비) |
| `/ability/[id]` | "구현 안 됨, 신규 생성 필요 (3순위)" | **구현됨** — `src/app/ability/[id]/page.tsx` (동일 패턴)                                    |

### 영향

- BA가 제시한 TOP 5 콘텐츠 우선순위 중 **1순위와 3순위가 모두 사실에 기반하지 않은 추정**
- 본 ADR의 "결정 1(MAU 30K 상향)" 자체는 사용자 동의에 따라 결정된 사항으로 유지되나, "결정 2(키워드 점유 KPI)"와 "결정 3(/type-effectiveness 의존도 분산)"의 **근거 보고서가 신뢰성을 잃음**
- 후속 작업(`/moves/[id]`, `/ability/[id]` 신규 구현) 자체가 무효 — 이미 구현된 페이지의 트래픽이 낮은 진짜 원인(콘텐츠/SEO/색인 문제)을 다시 진단해야 함

### 후속 조치

1. 본 ADR-0005는 **폐기** (대체 ADR 추후 작성)
2. 근거 보고서(`MI-BA-2026-05-26-podic-traffic-overtake.md`)는 상단 폐기 마크 표시 후 보존
3. MI/BA 에이전트 정의에 **상용 구현 확인 의무** 지침 추가 — 같은 오류 재발 방지
4. `service-overview.md`의 MAU 30K 목표는 **사용자 동의 기반**이므로 유지. 단 근거 ADR이 폐기되었음을 명시
5. 새 보고서/ADR은 **실제 상용 구현 상태를 확인한 후** 다시 작성

### 재발 방지

본 사건의 직접 원인은 BA 에이전트가 `src/app/` 라우트 구조를 직접 확인하지 않고 `metrics-baseline.md`의 통계만 보고 "기능 부재"를 추정한 것이다. 다음 위치에 재발 방지 지침을 추가했다.
...
## 아래 원본 내용은 폐기되었으나 기록 보존 목적으로 유지
