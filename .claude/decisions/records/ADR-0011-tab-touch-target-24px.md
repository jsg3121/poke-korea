...
- **상태**: 승인
- **날짜**: 2026-06-25 (2026-06-28 칩 확장 · 2026-06-29 입력 컨트롤 확장 · 2026-07-01 탭 모바일 24→36px)
- **담당**: jsg3121 + Claude
...
4. **적용 범위는 슬림 인터랙티브(탭·칩) + 입력 컨트롤/아이콘 버튼의 데스크톱 컴팩트**로
   ...
   - **탭(`TabItem`)**: 모바일 36px, 데스크톱 44px(`min-h-9 desktop:min-h-touch`).
     최초 24px(`touch-tab`)로 정했으나, 네비 바 조립 시 항목 높이가 컨테이너 높이와 맞아
     active 밑줄이 하단 경계선에 붙도록 36px로 올렸다(2026-07-01). 36px은 WCAG 2.2
     2.5.8(AA, 24px)을 여유롭게 충족하므로 "항목 간격 24px 전제"도 불필요해졌다.
     `touch-tab`(24px) 토큰은 미사용이 되어 제거했다.
...
- `TabItem`(`tabItemStyle.ts`) 베이스 높이가 `min-h-9 desktop:min-h-touch`가 된다 —
  모바일 36px, 데스크톱 44px. (최초 `touch-tab`(24px) 토큰을 썼으나 36px 전환으로 미사용이
  되어 토큰 제거.)
- **탭은 모바일 36px이라 WCAG 2.5.8 spacing 예외(간격 24px 전제)가 불필요**하다 —
  36px > 24px이므로 자체로 AA 충족. 네비 바 조립 시 항목 높이가 컨테이너 높이(36px)와 맞아
  active 밑줄이 하단 경계선에 붙는다.
