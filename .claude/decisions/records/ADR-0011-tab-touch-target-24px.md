# ADR-0011: 모바일 탭 항목 터치 타겟 24px 허용 (데스크톱 44px 유지, WCAG 2.2 AA, 간격 24px 전제)
...
4. **적용 범위는 모바일 탭으로 한정**한다. 데스크톱 탭, 그리고 버튼·CTA·기타 인터랙티브
   요소의 44px(`min-h-touch`) 규칙은 그대로 유지한다.
5. styling.md 터치 타겟 규칙에 이 예외를 명시한다.
...
- styling.md 터치 타겟 규칙에 모바일 탭 예외가 명시됐다.
- 데스크톱 탭, 그리고 버튼·CTA 등 나머지 요소의 44px 규칙은 변경 없음.
