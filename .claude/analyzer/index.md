...
**조회 결과는 이 폴더에 저장하지 마세요.** 어차피 `.gitignore`에 걸려 추적되지 않고, 시점이 고정된 파일이 쌓이면 API를 붙인 의미가 사라집니다. 필요하면 `--json=/tmp/...`로 스크래치패드에 저장하세요.
...
- **출처**: 네이버 애널리틱스 대시보드
- **갱신 주기**: 필요 시 수동 다운로드 (공개 API 없음)
- **주요 지표**: 방문 현황, 인기 페이지

> 국내 유입 교차 확인이 필요할 때만 받아 쓰고, 확인 후에는 지웁니다. 시점이 고정된 파일을 폴더에 쌓아두지 않습니다.
...
Search Console·GA4·애드센스를 API로 직접 조회합니다. zip·CSV를 수동으로 내려받아 올릴 필요가 없고, 조회 시점 기준 최신 데이터를 얻습니다.

### 왜 API인가

수동 다운로드 파일은 **받은 시점에 고정**됩니다. 실제로 애드센스 데이터가 7월 26일 기준에서 멈춰 있어 GA4·서치콘솔과 기간이 어긋났고, 교차 분석이 불가능했습니다. 또 CSV는 상위 N행만 담기는 경우가 많아 롱테일 분석이 안 됩니다 — API는 검색어 25,000행까지 한 번에 받을 수 있습니다.
