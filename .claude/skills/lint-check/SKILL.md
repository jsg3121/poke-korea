...
description: |
  ...
  DO NOT TRIGGER when: 코드 리뷰(code-review 사용), 자동 포맷팅 실행만 필요
...
```bash
npx prettier --check "src/**/*.{ts,tsx,css,json}" 2>&1
```

### 2. ESLint 검사

모든 소스 파일에 대해 린트 검사를 실행한다 (`next lint` 또는 직접 실행):

```bash
npm run lint 2>&1
# 또는 특정 경로만: npx eslint "src/**/*.{ts,tsx}" 2>&1
```
...
- Prettier 위반은 `npx prettier --write`로, ESLint 에러는 `npx eslint --fix`로 자동 수정 가능한 것과 수동 수정이 필요한 것을 구분하여 안내한다.
