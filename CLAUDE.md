...
**IMPORTANT**: 다음 신호가 나타나면 **논의가 끝나기를 기다리지 말고 그 시점에 ADR 작성을 먼저 제안**한다. 작성 여부는 사용자가 결정한다.

- 하나의 이슈에 **2개 이상의 해결 후보**를 놓고 장단점을 비교하게 된 경우
- **트레이드오프**(성능 vs 복잡도, SEO vs 구현 편의, 범위 vs 리스크 등)를 따지는 대화가 오간 경우
- 어떤 방안을 **검토 후 기각**하고 그 이유가 코드에 남지 않는 경우
- 기술·라이브러리 **도입 효과나 실효성**을 근거를 들어 판단한 경우
- 명세·공식 문서의 제약 때문에 **차선책을 택한** 경우

단순 버그 수정, 단일 해법만 존재하는 작업, 스타일 조정은 대상이 아니다. 판단 기준 상세는 `.claude/decisions/index.md`를 참조한다.

> **Why:** 트레이드오프 비교는 대화 중에 근거가 가장 선명한데, 그 맥락이 코드 diff에는 남지 않는다. "왜 A가 아니라 B인가"와 "왜 C를 기각했는가"가 사라지면 나중에 같은 검토를 반복하거나, 이미 기각된 방안을 다시 도입하게 된다. 논의가 끝난 뒤에 제안하면 근거를 되짚느라 비용이 커지므로, 신호가 보이는 시점에 바로 제안한다.
