 ...
 *
 * 섹션 순서: 미니 히어로(뒤로가기+기술 정보) → [상단 광고] → 버전 nav(sticky,
 * 전체 폭 배경) → 기술별 포켓몬 그리드 → [하단 광고]. 버전 nav가 전체 폭 sticky라
 * 히어로/그리드와 달리 gutter 래핑 밖에 두고, 나머지는 max-w-[1280px]+px-4로 가둔다.
 *
 * 광고(RES-004 재도입) 2개:
 * - 상단: 기술 설명(히어로) 아래·버전 탭바 위 디스플레이(모바일 320×100/PC
 *   970×250). 인아티클은 상단에 두면 콘텐츠처럼 보여 어색하므로 배너형을 쓴다.
 * - 하단: 포켓몬 그리드 소비 후 인아티클(구버전 CTR 1.81% 검증 위치).
 * 두 컴포넌트 모두 내부 useDevice로 PC·모바일 슬롯을 분기한다(기기별 성과 분리).
 *
  ...
  return (
    <>
      <div className="w-full max-w-[1280px] mx-auto px-4 py-6">
        ...
        />
        {/* 상단 광고 — 기술 설명(히어로) 아래·버전 탭바 위 디스플레이 */}
