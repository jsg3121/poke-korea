 ...
 * 규격화해 새로 만든다.
 *
 * - 자식 간격은 1rem(gap-4) 고정.
 * - 가로 스크롤 단서(M1): 카드가 고정 폭(w-56)이므로 컨테이너가 좁으면
 *   다음 카드가 자연히 살짝 보이는(peek) 구조 → 별도 calc 불필요.
 * - 호버 scale 여유: 카드가 hover로 커질 때(scale-105) 사방으로 넘쳐 Y축
 *   스크롤이 생기거나 양 끝 카드가 잘리지 않도록 사방 패딩(p-4)을 주고
 *   Y축 오버플로를 숨긴다.
...
const HorizontalScrollListComponent = ({
  ...
  return (
    <ul
      className={`w-full flex items-center gap-4 p-4 overflow-x-auto overflow-y-hidden ${
