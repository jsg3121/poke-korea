 ...
 *
 * id는 useId로 자동 생성해 충돌을 막는다. 색은 등록된 토큰만 사용한다.
  ...
  ({ label, disabled, ...inputProps }, ref) => {
    ...
    return (
      ...
      >
        ...
        <span
          aria-hidden="true"
          className="w-4 h-4 rounded border border-solid border-black-1 bg-white-3 absolute left-0 z-10 opacity-100 transition-opacity duration-300 peer-checked:opacity-0"
        />
        {/* 체크 — 포켓볼 (16px 박스에 맞춤). scale로만 등장/퇴장 */}
        <span
          aria-hidden="true"
          className="block w-4 h-4 absolute left-0 z-20 scale-0 transition-transform duration-300 will-change-transform peer-checked:scale-100"
        >
          <Ball />
        </span>
