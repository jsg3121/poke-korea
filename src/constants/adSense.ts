...
/**
 * 포켓몬 상세 습득 기술 탭(/detail/[id]/moves) 광고 슬롯 (RES-004).
 * 위치: 히어로 아래·학습법(레벨업/기술머신) 탭 위. 기술도감 상세(/moves/[id])와
 * 다른 페이지라 성과 분리를 위해 전용 슬롯을 새로 발급한다. 기기별 분리.
 *
 * 슬롯 값이 빈 문자열이면 렌더하지 않는다. 인아티클 유형으로 발급한다.
 * TODO(slot): 발급 후 아래 플레이스홀더('')를 실제 슬롯 ID로 교체한다.
 */
export const DETAIL_MOVES_SLOTS = {
  mobile: '3598746451', // 인아티클
  desktop: '6033604039', // 인아티클
} as const
