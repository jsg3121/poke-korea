...
- [ ] **메인 페이지**: 포켓몬 리스트의 모든 텍스트가 정상 렌더링
- [ ] **상세 페이지**: 포켓몬 이름, 설명, 능력치 등 모든 한글 텍스트 확인
- [ ] **검색 기능**: 검색어 입력 및 결과 표시 정상 동작
- [ ] **필터/정렬**: UI 텍스트가 모두 정상 표시
- [ ] **모달/팝업**: 이로치 관련 팝업 등의 텍스트 확인
- [ ] **에러 메시지**: 에러 발생 시 메시지 정상 표시

### 기능 테스트

- [ ] **폰트 로딩**: 개발자 도구 Network 탭에서 서브셋 폰트 로딩 확인
- [ ] **파일 크기**: 서브셋 폰트가 원본보다 작은 크기로 다운로드됨
- [ ] **캐싱**: 페이지 새로고침 시 폰트 캐싱 정상 동작
- [ ] **Fallback**: 특수 문자 입력 시 fallback 폰트로 렌더링

### 성능 테스트

- [ ] **Lighthouse 점수**: Performance 점수 유지 또는 개선
- [ ] **FCP**: First Contentful Paint 개선 확인
- [ ] **LCP**: Largest Contentful Paint 개선 확인
- [ ] **CLS**: Cumulative Layout Shift 유지 (0에 가까워야 함)
...
- [ ] **Safari**: 정상 렌더링
- [ ] **Edge**: 정상 렌더링
- [ ] **모바일 Safari (iOS)**: 정상 렌더링
- [ ] **Chrome Mobile (Android)**: 정상 렌더링

### 특수 케이스 테스트

- [ ] **긴 텍스트**: 여러 줄에 걸친 텍스트 렌더링
- [ ] **특수 기호**: 숫자, 영문, 기호가 혼합된 텍스트
- [ ] **느린 네트워크**: Throttling 환경에서 폰트 로딩 확인
