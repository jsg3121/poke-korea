...
> **작업 날짜**: 2026-01-02
> **브랜치**: `feature/1.28.0-image-optimization`
> **PR**: TBD
...
- 데스크톱: 160×160 표시, 240×240 다운로드
- 모바일: 140×140 표시, 180×180 다운로드
...
**효과**: 모바일 2열 그리드 × 3행 = 첫 화면 6개에만 high 적용
...
|-----|--------|-----------|-----------|---------|
| DPR=1 | 240×240<br>(~18KB) | 160×160<br>(~10KB) | - | **44% 감소** |
| DPR=2 | 240×240<br>(~18KB) | - | 320×320<br>(~28KB) | **선명도 33% 향상** |

#### 모바일 (10rem = 120px)

| DPR | Before | After (1x) | After (2x) | 개선 효과 |
|-----|--------|-----------|-----------|---------|
| DPR=1 | 180×180<br>(~14KB) | 120×120<br>(~8KB) | - | **43% 감소** |
| DPR=2 | 180×180<br>(~14KB) | - | 240×240<br>(~20KB) | **선명도 33% 향상** |
...
#### 데스크톱
- [ ] **리스트 페이지**: 모든 카드 이미지가 정상 렌더링
- [ ] **고해상도 모니터 (DPR=2)**: 이미지가 선명하게 표시
- [ ] **일반 모니터 (DPR=1)**: 이미지가 빠르게 로드
- [ ] **능력 검색 페이지**: PokemonByAbilityCard 이미지 확인
- [ ] **기술 검색 페이지**: PokemonBySkillCard 이미지 확인

#### 모바일
- [ ] **리스트 페이지**: 2열 그리드 이미지 정상 표시
- [ ] **레티나 디스플레이 (DPR=2)**: 선명한 이미지
- [ ] **스크롤 성능**: 이미지 lazy loading 정상 동작
- [ ] **능력/기술 페이지**: 모바일 크기 이미지 확인

### 기능 테스트

#### 네트워크 분석
- [ ] **개발자 도구 Network 탭**: srcset이 포함된 이미지 요청 확인
- [ ] **DPR=1 환경**: 1x 이미지만 다운로드
- [ ] **DPR=2 환경**: 2x 이미지만 다운로드
- [ ] **품질 파라미터**: URL에 `q=85` 포함 확인
- [ ] **캐싱**: 동일 이미지 재요청 시 캐시 사용

#### 우선순위 테스트
- [ ] **데스크톱 리스트**: 처음 15개만 fetchPriority="high"
- [ ] **모바일 리스트**: 처음 6개만 fetchPriority="high"
- [ ] **나머지 이미지**: loading="lazy" 적용
- [ ] **스크롤 시**: lazy 이미지가 뷰포트 근처에서 로드

### 성능 테스트

#### PageSpeed Insights
- [ ] **데스크톱 점수**: Performance 점수 확인
- [ ] **모바일 점수**: Performance 점수 확인
- [ ] **"Properly size images" 경고**: 해결 확인
- [ ] **"Serve images in next-gen formats"**: WebP 유지 확인
- [ ] **Lighthouse 보고서**: 이미지 관련 지표 개선 확인

#### Core Web Vitals
- [ ] **LCP (Largest Contentful Paint)**: 개선 확인
- [ ] **FCP (First Contentful Paint)**: 개선 확인
- [ ] **CLS (Cumulative Layout Shift)**: 0 유지 (width/height 명시)

#### 실제 사용 테스트
- [ ] **느린 3G 네트워크**: 이미지 로딩 속도 체감 확인
- [ ] **빠른 4G/5G 네트워크**: 고해상도 이미지 선명도 확인
- [ ] **데이터 절약 모드**: 1x 이미지 우선 선택 확인

### 크로스 브라우저 테스트

- [ ] **Chrome (DPR=1)**: 160px 이미지 로드
- [ ] **Chrome (DPR=2)**: 320px 이미지 로드
- [ ] **Firefox**: srcset 정상 동작
- [ ] **Safari (macOS)**: 레티나 디스플레이에서 2x 이미지
- [ ] **Safari (iOS)**: 모바일 크기 이미지 정상 렌더링
- [ ] **Edge**: 모든 기능 정상 동작

### 에러 케이스 테스트

- [ ] **이미지 로드 실패**: onError 핸들러 정상 동작
- [ ] **CDN 오류**: fallback 동작 확인
- [ ] **느린 네트워크**: 스켈레톤 UI 표시
- [ ] **imageSize 미제공**: 기본값 160px 적용
