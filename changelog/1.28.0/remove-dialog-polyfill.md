...
- useEffect 블록: 28줄 (각 컴포넌트당 14줄 × 2)
...
- [x] dialog-polyfill import 구문 완전 제거
- [x] useEffect 블록 완전 제거
- [x] package.json에서 dependency 제거 확인
- [x] npm install 정상 실행 (1 package removed)
- [ ] ESLint/Prettier 통과 (사용자 테스트 필요)
- [ ] TypeScript 컴파일 에러 없음 (사용자 테스트 필요)

### 기능 테스트

- [ ] "이로치란?" 버튼 클릭 시 모달 정상 오픈
- [ ] "이로치 포획률" 버튼 클릭 시 모달 정상 오픈
- [ ] 모달 닫기 버튼 정상 동작
- [ ] 모달 열릴 때 body 스크롤 잠금 정상 작동
- [ ] 모달 닫힐 때 body 스크롤 해제 정상 작동
- [ ] 모달 backdrop 클릭 시 닫힘 동작 확인
- [ ] ESC 키로 모달 닫힘 동작 확인
...
- [ ] Firefox 115+ 정상 동작
- [ ] Safari 15.4+ 정상 동작
- [ ] iOS Safari 15.4+ 정상 동작
- [ ] Edge 114+ 정상 동작

### 성능 테스트

- [ ] 번들 크기 약 7KB 감소 확인
- [ ] 페이지 로드 시 불필요한 polyfill 로드 제거 확인
- [ ] 네트워크 탭에서 dialog-polyfill.css 요청 없음 확인
